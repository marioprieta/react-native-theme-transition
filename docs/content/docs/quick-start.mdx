...
<Steps>
  <Step>
    ...
    your token keys (`'background' | 'card' | 'text' | …`). Both flow
    into `useTheme` automatically.

    By convention, name one token literally `background`. The engine
    looks for a token with that exact name and uses it as the root
    fill color behind your tree. If you skip it, the engine falls
    back to the first token in the theme — usually fine, but easy to
    surprise yourself when reordering. See
    [Root Background Color](/docs/api/create-theme-transition#root-background-color).
  </Step>
  ...
  <Step>
    ...
    won't appear in the overlay and may flash.
