---
title: useTheme
description: Read the painted theme, the user's preference, the mutator, and the transition flag from the nearest provider.
---
...
The hook is created by [createThemeTransition](/docs/api/create-theme-transition)
...
- **`theme`** describes what is currently painted on screen.
  ...
  [systemThemeMap](/docs/api/create-theme-transition#systemthememap)
...
| `options` | `SetThemeOptions` | Optional per-call configuration. See [SetThemeOptions](/docs/types#setthemeoptions) for every variant and field. |
...
<Callout type="info">
  Touch input is also blocked internally from the moment `setTheme`
...
See [Theme Picker](/docs/examples/theme-picker) for the
...
- [createThemeTransition](/docs/api/create-theme-transition). The factory that produces the hook.
- [SetThemeOptions](/docs/types#setthemeoptions). Every variant and field.
