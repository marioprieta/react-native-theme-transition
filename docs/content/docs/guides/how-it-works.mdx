---
title: How It Works
description: The snapshot-overlay technique behind smooth, native-feeling theme transitions in pure JavaScript.
---

Every transition is the same simple trick. Take a picture of what's
on screen, put it on top, change the colors underneath, then
animate the picture away. The picture is a Skia `SkImage`, the
animation runs on the UI thread via Reanimated, and the whole
pipeline is policy-free. There are no Reduce Motion checks, no
`setTimeout` fallbacks, no magic defaults beyond the per-kind
durations.
...
Most theme-transition libraries reach for a custom native module to
manipulate the view hierarchy directly. The snapshot approach needs
zero native code. Skia and Reanimated do everything, both ship in
modern Expo SDKs, and the result works in Expo Go.
...
- [setTheme reference](/docs/api/use-theme#settheme). Every option, every variant.
- [Callbacks](/docs/guides/callbacks). When each callback fires and why.
- [Troubleshooting](/docs/guides/troubleshooting). Common gotchas.
