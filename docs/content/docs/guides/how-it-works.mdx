...
8. Color tokens switched underneath — the provider updates its React context with new tokens, all children re-render with new colors while the overlay is still opaque
...
The animation duration (default 350ms) is independent of frame rate and runs via Reanimated
on the UI thread, matching the device's native refresh rate (60–120 FPS).
