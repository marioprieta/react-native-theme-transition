...
The same rule applies to `FlatList` and `SectionList`. Or rely on
the library's automatic root background, which paints the token
named `background` from the active theme behind the entire inner
tree.
