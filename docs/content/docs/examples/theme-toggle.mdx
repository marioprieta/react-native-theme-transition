...
- **`select()` instead of `setTheme()`** — the toggle thumb is a selection
  indicator, so it needs transition-safe timing. `select()` updates the
  highlight immediately and defers `setTheme` by one frame.
- **Plain React styles only** — no Reanimated for the thumb position. The
  visual state must update in React's commit cycle so the screenshot captures
  the correct position.
- `select()` includes a built-in press lock — if called during a transition,
  it's silently ignored. `disabled={isTransitioning}` provides visual feedback,
  but the internal lock prevents race conditions even without it.
