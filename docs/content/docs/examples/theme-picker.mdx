---
title: Theme Picker
description: A segmented control highlighted by `preference`, so the `'system'` row can win.
---
...
engine's `SETTLE.beforeCapture` window gives React time to paint the
new selection before the snapshot runs.
...
See [State Managers](/docs/recipes/zustand) for the hydration pattern
and [Persisted Preference](/docs/recipes/persistence) for the
AsyncStorage-only version.

## See Also

- [Checkmark List](/docs/examples/checkmark-list). iOS Settings-style list using the same pattern.
