...
Uses `select()` for flicker-free selection and rapid-press protection.
...
- `useTheme({ initialSelection: 'system' })` — seeds `selected` to `'system'` on first render. Without the override, `selected` defaults to the current theme name.
