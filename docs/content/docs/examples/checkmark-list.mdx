...
Uses `select()` for flicker-free selection and system mode support.
...
- `useTheme({ initialSelection: 'system' })` — seeds `selected` to `'system'` so "System" is checked on first render
