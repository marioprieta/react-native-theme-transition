---
title: Multiple Theme Scopes
description: Run two or more isolated theme transition systems in the same app, each with its own themes and provider.
---

`createThemeTransition` returns a self-contained
`{ ThemeTransitionProvider, useTheme }` API with no module-level
state. You can call it as many times as you want and each instance
is fully independent: separate Context, separate state, separate
animation pipeline.
...
```ts title="src/lib/marketing-theme.ts"
...
- [Migration from another theme system](/docs/recipes/migration). Patterns for moving an existing app onto the library.
