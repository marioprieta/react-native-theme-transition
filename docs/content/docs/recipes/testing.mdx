---
title: Testing Components That Use useTheme
description: Set up Jest mocks for Skia, Reanimated, and Worklets so components that read from the library can be tested headlessly.
---
...
mocks. This recipe shows the minimum mock setup that lets you mount
a themed component in a test.
...
// React Native core APIs the engine reads.
...
- [How transitions work](/docs/guides/how-it-works). The state machine these mocks stand in for.
- [Callbacks](/docs/guides/callbacks). The callback firing order tested above.
