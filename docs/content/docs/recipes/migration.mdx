...
description: Upgrade from 1.x to 2.0, or migrate to the library from plain Context, Zustand, Redux, MMKV, or React Navigation theming.
---
...
See [Installation](/docs/getting-started).
...
[useTheme](/docs/api/use-theme#theme-vs-preference) for the full
decision table.
...
The library now picks the `background` token from the active theme
automatically. If your background token is named differently, alias
it:
...
// 2.0: rename or alias to `background`
createThemeTransition({
  themes: {
    light: { bg: '#fff', background: '#fff' },
    dark:  { bg: '#111', background: '#111' },
  },
})
```
...
<Steps>
  <Step>**Install** the library and its peer dependencies. See [Getting Started](/docs/getting-started).</Step>
  <Step>**Create the theme file** that defines your tokens (include a `background` key) and calls `createThemeTransition`.</Step>
  <Step>**Wrap with the provider** at the root of your app.</Step>
  <Step>**Replace** color references with `useTheme().theme.colors`.</Step>
  <Step>**Remove** the old theme context/provider.</Step>
</Steps>
...
See [Expo Router](/docs/recipes/expo-router) for the same pattern
...
- [ ] Create the theme file with matching token keys; include a `background` token.
...
- [Expo Router](/docs/recipes/expo-router). Canonical navigation integration.
- [Persistence](/docs/recipes/persistence). Save the user's preference across launches.
