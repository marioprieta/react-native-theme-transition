...
description: Save the user's theme choice to AsyncStorage and restore it on app start without a double transition.
---
...
- [Troubleshooting: Double Transition on App Start](/docs/guides/troubleshooting#double-transition-on-app-start). The same pattern, diagnosed from the symptom side.
- [State Managers](/docs/recipes/zustand). Bridge with Zustand, Redux, or MMKV.
- [Theme Picker](/docs/examples/theme-picker). Segmented control built around `preference`.
- [Checkmark List](/docs/examples/checkmark-list). iOS Settings-style list.
