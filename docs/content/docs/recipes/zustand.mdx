...
description: Use Zustand, Redux, or MMKV to persist the user's theme pick across app launches.
---
...
<Callout type="info">
  `preference` updates synchronously before the engine's
  `SETTLE.beforeCapture` window, so the highlight paints before the
  snapshot runs. Reading the pill from the store directly would lag
  and show the wrong highlight during the transition.
</Callout>
...
<Callout type="warn">
  MMKV requires native modules and is **not compatible with Expo Go**.
...
- [Persisted Preference](/docs/recipes/persistence). The AsyncStorage-only version.
- [Theme Picker](/docs/examples/theme-picker). The picker UI built around `preference`.
