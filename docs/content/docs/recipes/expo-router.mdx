---
title: Expo Router
description: Wire up Expo Router with a theme that propagates to navigation, status bar, and safe-area backgrounds on iOS.
---
...
<Callout type="warn">
  React Native's built-in `Modal` renders in a separate native window
...
- [React Navigation example](/docs/examples/react-navigation). Same pattern for `NavigationContainer`.
- [useTheme](/docs/api/use-theme). The hook reference.
- [Troubleshooting](/docs/guides/troubleshooting). iOS safe-area background fix.
