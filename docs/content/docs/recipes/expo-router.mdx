...
- `headerShown: false` is used here for simplicity — it's not required by the library. Headers inside the provider are captured in the screenshot normally.

### Nesting order

```
GestureHandlerRootView
  └─ ThemeTransitionProvider     ← captures everything inside for screenshots
      └─ ThemeProvider            ← from @react-navigation/native, feeds colors to headers/tabs
