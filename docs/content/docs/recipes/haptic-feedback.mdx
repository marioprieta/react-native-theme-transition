---
title: Haptics & Analytics
description: Add haptic feedback and analytics tracking to theme transitions via the lifecycle callbacks.
---
...
[Callbacks](/docs/guides/callbacks) for the full ordering.
...
<Callout type="info">
  `onTransitionStart` and `onTransitionEnd` fire only for animated
...
<Callout type="warn">
  Do not change visual styles based on `isTransitioning`, e.g.
...
- [Callbacks](/docs/guides/callbacks). Full lifecycle ordering and edge cases.
- [createThemeTransition](/docs/api/create-theme-transition#arguments). Config-level callback fields.
- [SetThemeOptions](/docs/types#setthemeoptions). Per-call callback fields.
