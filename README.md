...
Pass `initialTheme="system"` to follow the OS appearance with zero-flash on the first frame:
...
Store the user's preference as `'light' | 'dark' | 'system'` (or your custom theme names + `'system'`). Pass it directly to `initialTheme` — no branching needed:

```tsx
import { ThemeTransitionProvider } from './theme';
import { useStoredPreference } from './storage';
...
In a settings screen, use `setTheme` to switch modes at runtime:
...
Map your color tokens to React Navigation's theme shape. Adjust the token names to match your own theme definition:
...
function App() {
  ...
  const navigationTheme = {
    dark: name === 'dark',
    colors: {
      primary: colors.primary,       // adjust to your token names
